...
async function doExportHTML() {
  try {
    ...
    // draw full map — exact tile dimensions
...
// MIN_SCALE: cover zoom — map always fills screen, no black/green border visible
function calcMin(){return Math.max(window.innerWidth/${w},window.innerHeight/${h});}
let MIN_SCALE=calcMin(),scale=MIN_SCALE;
...
window.addEventListener('resize',()=>{MIN_SCALE=calcMin();if(scale<MIN_SCALE)applyScale(MIN_SCALE,wrap.clientWidth/2,wrap.clientHeight/2);});
...
function fitAreas(title){const els=[...document.querySelectorAll('.hs-area[data-title="'+title+'"]')];if(!els.length)return;let minX=Infinity,minY=Infinity,maxX=0,maxY=0;els.forEach(el=>{const ox=+el.dataset.ox,oy=+el.dataset.oy,ow=+el.dataset.ow,oh=+el.dataset.oh;minX=Math.min(minX,ox);minY=Math.min(minY,oy);maxX=Math.max(maxX,ox+ow);maxY=Math.max(maxY,oy+oh);});const PAD=80,sw=wrap.clientWidth-PAD*2,sh=wrap.clientHeight-PAD*2;const z=Math.min(sw/(maxX-minX||1),sh/(maxY-minY||1),4);applyScale(Math.max(MIN_SCALE,z),wrap.clientWidth/2,wrap.clientHeight/2);const cx=(minX+(maxX-minX)/2)*scale,cy=(minY+(maxY-minY)/2)*scale;let n=0;(function go(){wrap.scrollLeft=cx-wrap.clientWidth/2;wrap.scrollTop=cy-wrap.clientHeight/2;if(++n<6)setTimeout(go,120);})();}
// ── menu ──
...
// ── hash navigation ──
function applySpotHash(){const h=window.location.hash;if(!h.startsWith('#spot='))return;const parts=h.slice(6).split(',');if(parts.length<2)return;const col=parseInt(parts[0]),row=parseInt(parts[1]),z=parts.length>=3?parseFloat(parts[2]):1;if(isNaN(col)||isNaN(row)||isNaN(z))return;scale=Math.max(MIN_SCALE,Math.min(8,z));inner.style.transform='scale('+scale+')';sizer.style.width=(${w}*scale)+'px';sizer.style.height=(${h}*scale)+'px';const sx=Math.max(0,col*${TS}*scale-wrap.clientWidth/2),sy=Math.max(0,row*${TS}*scale-wrap.clientHeight/2);let n=0;(function go(){wrap.scrollLeft=sx;wrap.scrollTop=sy;if(++n<8)setTimeout(go,150);})();}
...
window.addEventListener('load',()=>{applyScale(MIN_SCALE,0,0);applySpotHash();applyAreaHash();});
