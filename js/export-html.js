...
#notifPopup{display:none;position:fixed;z-index:70;background:rgba(13,17,23,0.75);backdrop-filter:blur(16px);border:2px solid var(--nc,#58a6ff);border-radius:12px;padding:14px 40px 14px 14px;max-width:min(88vw,360px);font:13px/1.6 sans-serif;color:#e6edf3;box-shadow:0 4px 24px rgba(0,0,0,0.5);}
...
function _startObjBlink(el){_stopObjBlink();const ox=+el.dataset.ox,oy=+el.dataset.oy,ow=+el.dataset.ow,oh=+el.dataset.oh;const ov=document.createElement('canvas');ov.width=${w};ov.height=${h};ov.style.cssText='position:absolute;top:0;left:0;pointer-events:none;z-index:15;';inner.appendChild(ov);_objBlinkOv=ov;const ctx=ov.getContext('2d');ctx.lineWidth=2;ctx.lineCap='square';let t=0;function frame(){ctx.clearRect(0,0,ov.width,ov.height);t+=0.05;const alpha=0.5+0.5*Math.sin(t*3);ctx.strokeStyle='rgba(255,220,80,'+alpha.toFixed(2)+')';ctx.strokeRect(ox+1,oy+1,ow-2,oh-2);_objBlinkRaf=requestAnimationFrame(frame);}frame();}
function _stopObjBlink(){if(_objBlinkRaf){cancelAnimationFrame(_objBlinkRaf);_objBlinkRaf=null;}if(_objBlinkOv){_objBlinkOv.remove();_objBlinkOv=null;}}
