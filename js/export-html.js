...
#mdlTerm{position:fixed;top:0;left:0;right:0;height:46vh;z-index:998;display:flex;flex-direction:column;background:rgba(0,0,0,.45);backdrop-filter:blur(6px) saturate(.75);-webkit-backdrop-filter:blur(6px) saturate(.75);border-bottom:1px solid rgba(0,255,136,.15);box-shadow:0 6px 32px rgba(0,0,0,.55);transform:translateY(-100%);transition:transform 220ms cubic-bezier(.22,1,.36,1);overflow:hidden;}
...
.tm-wm img{width:52%;max-width:260px;opacity:.07;filter:grayscale(1);user-select:none;}
...
<body>
<div id="topbar">
  <button id="termBtn" onclick="toggleTerm()"><span class="tm-tilde">~</span>${title}</button>
  <span id="mapTitle">${title}</span>
</div>
<div id="mapWrap">
...
<div id="mdlTerm">
  <div class="tm-wm">
    <img src="logo.png" alt="" id="tmLogo" onerror="document.getElementById('tmLogo').style.display='none';document.getElementById('tmLogoTxt').style.display='block'">
    <span class="tm-wm-txt" id="tmLogoTxt" style="display:none">MDELO</span>
  </div>
  <div class="tm-hdr">
    <span class="tm-path"><em>~/</em>mdelo</span>
    <div class="tm-ctrl">
      <button id="tmFullBtn" onclick="tmToggleFull()" title="სრული ეკრანი">⛶</button>
      <button onclick="tmClear()" title="გასუფთავება">⌫</button>
      <button onclick="closeTerm()">✕</button>
    </div>
  </div>
  <div id="tmOut"></div>
  <div class="tm-ir">
    <span class="tm-pr">~/mdelo $</span>
    <input id="tmIn" autocomplete="off" spellcheck="false" placeholder="ბრძანება...">
    <span id="tmHint"></span>
  </div>
</div>
...
window.addEventListener('load',()=>{applySpotHash();applyAreaHash();_tmInit();});
