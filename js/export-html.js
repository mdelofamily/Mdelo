...
function _startRealtime(){try{const ws=new WebSocket('wss://miqenmsgwkkmtxwwbxzo.supabase.co/realtime/v1/websocket?apikey='+SUPA_KEY+'&vsn=1.0.0');ws.onopen=()=>{ws.send(JSON.stringify({topic:'realtime:public:notifications',event:'phx_join',payload:{config:{broadcast:{self:true},presence:{key:''},postgres_changes:[{event:'*',schema:'public',table:'notifications'}]}},ref:'1'}));};ws.onmessage=e=>{try{const msg=JSON.parse(e.data);if(msg.event==='postgres_changes'||(msg.payload&&msg.payload.data&&msg.payload.data.type)){loadNotifs();}}catch(_){}};ws.onclose=()=>{setTimeout(_startRealtime,3000);};ws.onerror=()=>{ws.close();};}catch(_){}}
...
window.addEventListener('load',()=>{loadNotifs();_startRealtime();applySpotHash();applyAreaHash();});
