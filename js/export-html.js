...
let _objBlinkRaf=null,_objBlinkEl=null;
function _startObjBlink(el){_stopObjBlink();_objBlinkEl=el;let t=0;function frame(){t+=0.04;const a=(0.5+0.5*Math.sin(t*2.5)).toFixed(2);const b=(4+4*Math.sin(t*2.5)).toFixed(1);el.style.filter='drop-shadow(0 0 '+b+'px rgba(255,220,80,'+a+'))';_objBlinkRaf=requestAnimationFrame(frame);}frame();}
function _stopObjBlink(){if(_objBlinkRaf){cancelAnimationFrame(_objBlinkRaf);_objBlinkRaf=null;}if(_objBlinkEl){_objBlinkEl.style.filter='';_objBlinkEl=null;}}
