...
// Returns { ok:true, data } on success, { ok:false, network:true } for a
// genuine connectivity failure (fetch threw — DNS blip, dropped wifi mid-
// request, timeout), or { ok:false, network:false, status } when the
// server actually answered and rejected the refresh_token (400 invalid_
// grant — token expired/revoked/already-used). This distinction matters:
// see the caller below.
async function _authRefresh(refresh_token) {
  try {
    ...
    return { ok: false, network: false, status: r.status };
...
//
// CRITICAL: only a genuine server rejection wipes the session. A network
...
// transient connectivity blip they may never even have noticed (the exact
// "logged out the next day" bug — the token's own timer had already run
// out overnight, so the very next refresh attempt after reopening the tab
// was also the very first time the failure got noticed and acted on).
